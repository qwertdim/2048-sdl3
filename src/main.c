...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    if (as->mute)
    {
        const SDL_FRect src = {461.f, 471.f, 38.f, 40.f};
        const SDL_FRect dst = {493.f, 53.f + as->safe_area.y, 38.f, 40.f};
        SDL_RenderTexture(as->renderer, as->texture, &src, &dst);
    }
    ...
    if ((as->win && !as->continue_game) || as->game_over)
    {
        ...
        const SDL_FRect dst = {32.f, 701.f, 512.f, 66.f};
        SDL_RenderTexture(as->renderer, as->texture, &src, &dst);
    }
    ...
    if (as->pressed_button)
    {
        if (now < as->stop_anim_at - DURATION)
        {
            ...
            const SDL_FRect src = {179.f, 102.f, 154.f, 171.f};
            SDL_RenderTexture(as->renderer, as->texture, &src, &dst);
        }
        else
        {
...
static SDL_AppResult ReadGameData(AppState *appstate)
{
...
static bool ReadSave(AppState *appstate)
{
    bool flag = true;
...
static void WriteSave(AppState *appstate)
{
    SDL_Storage *user = SDL_OpenUserStorage("qwertdim", "game_2048", 0);
    if (user == NULL)
    {
